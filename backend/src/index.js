...
const DEFAULT_FRONTEND_ORIGINS = ["http://localhost:5173", "http://127.0.0.1:5173", "http://localhost:3000"];
const FRONTEND_ORIGINS = (process.env.FRONTEND_URL || "")
  .split(",")
  .map((origin) => origin.trim())
  .filter(Boolean);
const ALLOWED_ORIGINS = FRONTEND_ORIGINS.length ? FRONTEND_ORIGINS : DEFAULT_FRONTEND_ORIGINS;
...
// Health check
app.get(["/health", "/api/health"], (req, res) => res.json({ status: "ok", env: process.env.NODE_ENV }));
