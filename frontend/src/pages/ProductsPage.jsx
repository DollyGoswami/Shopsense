...
export default function ProductsPage({
  ...
  const AUTO_REFRESH_MAX   = 30;
  const DISPLAY_PAGES_LIMIT = 10;

  const { products, loading, error, search, pagination, clearResults } = useProducts();
  ...
  }, [applied, page, sort, clearTimer, search, startPolling, getSourceSignature, clearResults]);
  ...
  const hasSearchQuery = applied.query.trim().length >= MIN_QUERY_LENGTH;
  const showBlockingLoader = loading || ((isUpdating || isLive) && visibleProducts.length === 0);

  return (
    <div className="ps-root">
      <style>{GLOBAL_CSS}</style>

      {hasSearchQuery && <FilterFAB activeCount={activeFilterCount} onClick={() => setFilterOpen(true)} />}
      <FilterDrawer
      ...
      <div style={{ maxWidth: 1440, margin: "0 auto", padding: "clamp(24px,4vw,56px) clamp(16px,4vw,52px)" }}>
        ...
        <div style={{ marginBottom: 44 }}>
          ...
          <p style={{
            ...
            Search for a product to compare live listings, prices, and AI signals.
          </p>
        </div>
        ...
        <form
          ...
          style={{
            ...
            padding: "14px",
            borderRadius: 16,
            background: "rgba(240,235,220,0.035)",
            border: "1px solid rgba(200,169,110,0.14)",
          }}
        >
          <div style={{
        ...
        {/* ── Toolbar ── */}
        {hasSearchQuery && <div className="ps-toolbar-card" style={{ marginBottom: 16 }}>
        ...
        {showBlockingLoader ? (
          ...
          <div style={{
            ...
            <p style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 30, fontWeight: 400, color: "rgba(240,235,220,0.82)", marginBottom: 10 }}>
              {hasSearchQuery ? "No products found yet" : "Start with a search"}
            </p>
            <p style={{ fontSize: 13, color: "rgba(240,235,220,0.35)", maxWidth: 300, margin: "0 auto 32px", lineHeight: 1.75 }}>
              {hasSearchQuery
                ...
                : "Products will appear here only after you search for something."}
            </p>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 12, flexWrap: "wrap" }}>
              <button className="ps-btn-gold" onClick={() => handleSearch()} disabled={query.trim().length < MIN_QUERY_LENGTH}>
                Search {query || "products"}
              </button>
              {(query || hasSearchQuery || activeFilterCount > 0) && (
                <button className="ps-btn-ghost" onClick={resetAll}>Clear</button>
              )}
            </div>
          </div>
