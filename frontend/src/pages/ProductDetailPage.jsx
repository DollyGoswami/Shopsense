...
const SOURCE_HOST_HINTS = {
  amazon: "amazon.",
  flipkart: "flipkart.com",
  myntra: "myntra.com",
  apollo_pharmacy: "apollopharmacy.in",
};
...
function extractAsin(urlValue = "") {
  const match = String(urlValue).match(/\/(?:dp|gp\/product)\/([A-Z0-9]{10})/i);
  return match?.[1]?.toUpperCase() || null;
}

function normalizeMarketplaceUrl(urlValue = "", sourceValue = "") {
  const raw = String(urlValue || "").trim();
  if (!raw) return null;

  const source = String(sourceValue || "").trim().toLowerCase();
  const baseUrl = SOURCE_BASE_URLS[source] || window.location.origin;

  let safeRaw = raw;
  const duplicateScheme = safeRaw.slice(8).search(/https?:\/\//i);
  if (duplicateScheme >= 0) safeRaw = safeRaw.slice(8 + duplicateScheme);

  let parsed;
  try {
    parsed = new URL(safeRaw, baseUrl);
  } catch {
    return null;
  }
  ...
  const hostHint = SOURCE_HOST_HINTS[source];
  if (hostHint && !parsed.hostname.toLowerCase().includes(hostHint)) return null;

  parsed.hash = "";

  if (source === "amazon") {
    const asin =
      String(urlValue).match(/[?&]asin=([A-Z0-9]{10})/i)?.[1]?.toUpperCase() ||
      extractAsin(parsed.pathname);
    if (!asin) return null;
    return `${SOURCE_BASE_URLS.amazon}/dp/${asin}`;
  }

  if (source === "flipkart") {
    const pid = parsed.searchParams.get("pid");
    if (!pid && !parsed.pathname.includes("/p/")) return null;
    ...
    if (lid) clean.searchParams.set("lid", lid);
    return clean.toString();
  }

  if (source === "apollo_pharmacy" && !/\/(otc|medicine|products)\//i.test(parsed.pathname)) {
    return null;
  }

  if (source === "myntra" && /\/(?:$|gateway|search)/i.test(parsed.pathname)) {
    return null;
  }
...
function looksLikeMatchingUrl(product = {}, urlValue = "") {
  ...
  const normalizedUrl = normalizeMarketplaceUrl(urlValue, source);
...
function resolveProductUrl(product = {}, referenceProduct = product) {
  ...
  const candidates = [
    ...
    .map((candidate) => normalizeMarketplaceUrl(candidate, source))
    .filter(Boolean);

  const isSameListing = Boolean(product?._id && referenceProduct?._id && String(product._id) === String(referenceProduct._id));
  const directCandidate = candidates[0];

  if (directCandidate && (isSameListing || isComparableListing(referenceProduct, product))) {
